...
impl<T> Local<T> {
    ...
    fn remove_node(&self, to_remove: &WaiterNode<T>) {
        ...
        if let Some(prev) = prev {
            ...
            debug_assert_eq!(prev_head, Some(NonNull::from(to_remove)),);
        }
    }
}
...
impl WaiterQueue<()> {
    ...
    pub async fn wait_for<R>(&self, mut condition: impl FnMut() -> Option<R>) -> R {
        ...
        let condition = UnsafeCell::new(condition);
        ...
        loop {
            WaitUntil {
                ...
                waiter: &unsafe { self.wait() },
                condition: || {
                    let condition = unsafe { &mut *condition.get() };
                    if let Some(r) = condition() {
                        unsafe {
                            *result.get() = Some(r);
                        }
            ...
            if let Some(r) = unsafe { &mut *result.get() }.take() {
                return r;
            }
        }
    }
...
impl<'a, T: IFulfillment> Waiter<'a, T> {
    ...
    fn register(
        ...
        if local.nodes.get().is_some() {
            ...
            self.waiter_queue.lock().add_waiter(waiter_node_ptr);
            ...
            if let Some(fulfillment) = try_fulfill() {
                self.waiter_queue
                    .lock()
                    .notify(fulfillment.inner, fulfillment.count)
            } else {
                None
            }
        }
            ...
            if let Some(mut fulfillment) = waiter_node.with_state(update_state) {
                ...
                while fulfillment.count > 1 && waiter_node.local_prev.get().is_some() {
                    if self
                        .as_ref()
                        .waiter_queue
                        .notify_one_local(fulfillment.take_one())
                        .is_some()
                    {
                ...
                if let Some(upgrade_waiter_node) = next_oldest_local_waiter {
                    ...
                    *upgrade_waiter_node_ref.state.lock() =
                        match unsafe { &*upgrade_waiter_node_ref.local_state.get() } {
                    ...
                    self.as_ref()
                        .waiter_queue
                        .lock()
                        .add_waiter(upgrade_waiter_node);
                    unsafe { upgrade_waiter_node.as_ref() }
                        .local_lifecycle
                    ...
                    'try_fulfill: while let Some(new_fulfillment) = try_fulfill() {
                        ...
                        while fulfillment.count > 1 {
                            if let Some(unused_fulfillment) = self
                                .as_ref()
                                .waiter_queue
                                .notify_one_local(fulfillment.take_one())
                            {
